...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(len(words)):
            POS_list[words[i]]=pos[i]

        for i in range(-1,-4,-1):
            try:
                if state.stack[i] and POS_list[words[state.stack[i]]] != None:
                    if POS_list[words[state.stack[i]]] == 'CD':
                        input_list.append(self.word_vocab['<CD>'])
                    elif POS_list[words[state.stack[i]]] == 'NNP':
        ...
        for i in range(-1,-4,-1):
            try:
                if state.buffer[i] and POS_list[words[state.buffer[i]]] != None:
                    if POS_list[words[state.buffer[i]]] == 'CD':
                        input_list.append(self.word_vocab['<CD>'])
                    elif POS_list[words[state.buffer[i]]] == 'NNP':
                        input_list.append(self.word_vocab['<NNP>'])
                    elif words[state.buffer[i]].lower() in self.word_vocab:
                        input_list.append(self.word_vocab[words[state.buffer[i]].lower()])
                    else:
                        input_list.append(self.word_vocab['<UNK>'])
                else:
                    input_list.append(self.word_vocab['<ROOT>'])
    ...
    def get_output_representation(self,output_pair):  
        ...
        output_list = self.output_labels[output_pair]
        return tf.keras.utils.to_categorical(output_list, num_classes=91)
